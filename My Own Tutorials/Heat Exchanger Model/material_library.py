...
MATERIALS = {
    ...
    'Inconel617': {
        # Permeability: INL/EXT-10-19387 (2010), "Hydrogen Permeability of
        # Incoloy 800H, Inconel 617, and Haynes 230 Alloys", Table 5 average:
        #   K0 = 0.539 cm3(STP)/cm/s/atm^0.5, Q = 21.3 kcal/mol
        #   -> Phi0 = 7.56e-6 mol(H2)/m/s/Pa^0.5, EPhi = 89.1 kJ/mol
        #   measured 650-950 C at p_H2 = 1e-3 - 1e-2 atm (low-pressure regime,
        #   directly relevant to HX tritium duty). Cross-check (same report,
        #   Table 1): Schmidt K0=0.228/Q=18.9 kcal/mol agrees within the
        #   usual permeation scatter.
        # Diffusivity: no IN617-specific D found; Ni-base proxy (Inconel 600,
        #   Rota et al.): D = 1.3e-2 exp(-50.6 kJ/mol /RT) cm2/s
        #   -> D0 = 1.3e-6 m2/s, ED = 50.6 kJ/mol.
        # Solubility: derived KS0 = Phi0/D0 so that Phi = D*KS exactly.
        #   NOTE: the D/KS split is uncertain; steady-state permeation flux
        #   depends only on the product Phi, which is measured.
        'D0':    1.30e-6,   'ED':   50.6,
        'KS0':   5.81,      'ES':   38.5,
        'Phi0':  7.56e-6,   'EPhi': 89.1,
        'T_min': 923,       'T_max': 1223,
        'note': (
            'HX structural alloy. Permeability measured by INL (650-950 C); '
            'use below 650 C is Arrhenius EXTRAPOLATION (flagged by T-range '
            'warning). Bare-metal values = conservative. In-situ oxide layers '
            'suppress permeation by ~100x for IN617 (INL/EXT-10-19387) — '
            'model via f_phi ~ 0.01. INL FY11/12 tritium tests indicate '
            'transition toward surface-limited permeation at very low p_T2 '
            '(~1e-2 Pa); at ~10 Pa diffusion-limited is the right regime, '
            'but check the Lambda diagnostic.'
        ),
    },
